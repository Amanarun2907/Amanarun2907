<h1 align="center">Hi 👋, I'm Aman Jain</h1>

<h3 align="center">
🚀 Machine Learning Engineer | AI Research Enthusiast | Data Science Explorer
</h3>

<p align="center">
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=25&pause=1000&color=00F7FF&center=true&vCenter=true&width=1000&lines=Machine+Learning+%7C+Deep+Learning+%7C+AI+Research;Computer+Vision+%7C+Time+Series+%7C+Explainable+AI;Building+Intelligent+Systems+for+Real-World+Impact;Transforming+Data+into+AI-Powered+Solutions;Research+•+Innovation+•+Intelligence+•+Impact" alt="Typing SVG" />
</p>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Amanarun2907&label=Profile%20Views&color=0e75b6&style=for-the-badge" alt="Profile Views" />
</p>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=200&section=header&text=Aman%20Jain&fontSize=50&fontColor=ffffff&animation=fadeIn&fontAlignY=35" />

</div>

---

<img align="right" alt="Coding" width="350" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif">

# 🧠 About Me

🎓 B.Tech Computer Science & Engineering Student at **BML Munjal University**  
📊 Specializing in **Machine Learning, Artificial Intelligence & Data Science**  
🔬 Passionate about developing intelligent systems that solve real-world problems  
⚡ Research-oriented developer focused on impactful AI innovation  

🌱 Currently Exploring:
- Advanced Deep Learning Architectures
- Large Language Models (LLMs)
- Explainable AI Systems
- Financial Intelligence Platforms
- AI for Agriculture & Sustainability
- Time Series Forecasting
- Multi-Agent AI Systems

🏆 Reliance Foundation Scholar & Student Mentor  
🏅 Top Performer — LNMIIT HackCrux 2025  
📈 CGPA: **9.23 / 10.0**  
🚀 Believer in continuous learning, research & innovation

---

# 🚀 Tech Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=python,java,tensorflow,pytorch,opencv,mysql,git,github,vscode,linux&theme=dark" />

</p>

---

# 💻 Technical Expertise

## 🤖 Machine Learning
- Supervised & Unsupervised Learning
- Regression & Classification
- Clustering & PCA
- Ensemble Learning
- Feature Engineering
- Model Optimization
- Hyperparameter Tuning

---

## 🧠 Deep Learning
- ANN (Artificial Neural Networks)
- CNN (Convolutional Neural Networks)
- RNN, LSTM & GRU
- Transfer Learning
- Fine-Tuning Deep Learning Models

---

## 👁️ Computer Vision
- YOLOv8
- U-Net
- Object Detection
- Image Classification
- Semantic Segmentation
- Image Augmentation

---

## 📊 Data Science & Analytics
- Exploratory Data Analysis (EDA)
- Statistical Analysis
- Hypothesis Testing
- Data Cleaning & Visualization
- Predictive Modeling

---

## 📈 Time Series Analysis
- ARIMA
- SARIMAX
- Forecasting Models
- Trend & Seasonality Analysis

---

## 🔍 Explainable AI
- Grad-CAM
- SHAP
- LIME

---

# 🌟 Featured Projects

# 💹 SARTHAK NIVESH — AI Financial Intelligence Platform

🚀 Developed an AI-powered financial intelligence platform integrating:

✅ Real-time Market Analytics  
✅ IPO Analysis  
✅ SIP Planning  
✅ Portfolio Optimization  
✅ Risk Assessment  
✅ Explainable Investment Insights  

🧠 Designed a hierarchical multi-agent AI system using:
- LLMs (Groq Llama 3.3)
- Sentiment Analysis
- Financial Intelligence Pipelines

📌 Objective:
To simplify intelligent financial decision-making using AI-powered analytics.

---

# 🏛️ VIRASAT — Heritage Monument Recognition System

📸 AI-powered heritage intelligence system using:
- MobileNetV2
- Transfer Learning
- Computer Vision
- Vision-Language AI

🎯 Achievements:
- Monument recognition across 30 categories
- 96.89% Top-5 Accuracy

✨ Features:
- Historical Insight Generation
- Interactive AI Chatbot
- Personalized Heritage Exploration
- Video Analysis Support

📌 Objective:
Promoting Indian heritage through AI-driven digital experiences.

---

# 🌾 KRISHI SAHAYAK — AI Agriculture Assistant

🌱 Smart agriculture system developed using:
- YOLOv8
- U-Net
- NDVI Analytics
- Multi-task CNNs

✨ Features:
✅ Crop Health Monitoring  
✅ Pest Detection  
✅ Precision Agriculture Insights  
✅ Explainable AI Integration  

🔍 Applied:
- Grad-CAM
- SHAP
- LIME

📌 Objective:
Empowering farmers with AI-driven agricultural intelligence.

---

# 🔋 BEV Charging Time Prediction

📊 Conducted applied research on:
- Battery Electric Vehicle Charging Prediction
- Time Series Forecasting
- ML-based Charging Analytics

🧠 Models Used:
- SARIMAX
- LSTM
- Regression Models

📌 Objective:
Enhancing smart mobility & EV infrastructure analytics.

---

# 📈 MPCE Prediction System

📊 Built an economic analytics platform for:
- Monthly Per Capita Expenditure Prediction
- Government-focused Economic Insights
- ML-based Decision Support

✨ Features:
✅ Feature Engineering  
✅ PCA Optimization  
✅ Streamlit Dashboard  
✅ Comparative Model Evaluation  

📌 Objective:
Using AI for economic planning & policy analysis.

---

# 🌦️ IoT Weather Monitoring System

🌍 Developed a smart environmental monitoring system using:
- ESP32
- DHT11
- BMP180
- Blynk Cloud

📊 Features:
- Real-time Weather Monitoring
- Cloud Integration
- Forecasting Analytics
- IoT Dashboard

📌 Objective:
Building intelligent environmental monitoring systems.

---

# 💼 Experience

## 🏢 Data Science Intern — Celebal Technologies
📅 June 2025 – August 2025

✨ Worked on:
- End-to-End ML Pipelines
- Data Preprocessing
- Feature Engineering
- Customer Lifetime Value Prediction
- Business Analytics Systems

---

## 🔬 Research Intern — CADCS
📅 June 2025 – July 2025

✨ Conducted research on:
- EV Charging Prediction
- Machine Learning Models
- Time Series Forecasting
- Energy & Mobility Analytics

---

# 🏆 Achievements & Leadership

🏅 Reliance Foundation Scholar  
🏅 Scholar at Saraswati Education Support Trust  
🏅 Top 10 Position — LNMIIT HackCrux 2025  
🏅 NSS Operations Team Member  
🏅 Theatre Competition Winner  
🏔️ Successfully Summited Patalsupi Trek (4200m)  
🏅 Student Mentor at Reliance Foundation  

---

# 📊 GitHub Analytics

<p align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=Amanarun2907&show_icons=true&theme=github_dark&hide_border=true&bg_color=00000000&include_all_commits=true&count_private=true" />

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Amanarun2907&layout=compact&theme=github_dark&hide_border=true&bg_color=00000000" />

</p>

---

# 🔥 GitHub Streak

<p align="center">

<img src="https://streak-stats.demolab.com?user=Amanarun2907&theme=github-dark-blue&hide_border=true&background=00000000" />

</p>

---

# 📈 Contribution Graph

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Amanarun2907&theme=github-compact&hide_border=true&bg_color=00000000" />

</p>

---

# 🏅 GitHub Trophies

<p align="center">

<img src="https://github-profile-trophy.vercel.app/?username=Amanarun2907&theme=algolia&no-frame=true&row=1&column=7" />

</p>

---

# 🌐 Connect With Me

<p align="center">

<a href="https://github.com/Amanarun2907">
<img src="https://img.shields.io/badge/GitHub-121013?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/aman-jain-07449a221">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

</p>

---

# 💭 Philosophy

<p align="center">

✨ <i>"Transforming data into intelligent systems that create real-world impact."</i> ✨

</p>

---

<div align="center">

## 🚀 Research • Innovation • Intelligence • Impact 🚀

</div>

---

<p align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=120&section=footer"/>
</p>
